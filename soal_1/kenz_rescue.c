...
char filepath[1000]; snprintf(filepath, sizeof(filepath), "%s/%d.txt", source_dir, i);
...
if (!first) strncat(fragments, " ", sizeof(fragments) - strlen(fragments) - 1); strncat(fragments, val, sizeof(fragments) - strlen(fragments) - 1);
       first = 0;
            }
        }
...
if (written < 0) return NULL; *out_len = (size_t)written;
...
size_t len = 0;
 char *content = build_tujuan(&len); stbuf->st_size = (off_t)len;
        free(content);
...
if (dp == NULL) return -errno;
 ...
 struct dirent *de; filler(buf, ".", NULL, 0); filler(buf, "..", NULL, 0);

    
while ((de = readdir(dp)) != NULL) {
        

struct stat st; memset(&st, 0, sizeof(st)); st.st_ino  = de->d_ino; st.st_mode = de->d_type << 12; filler(buf, de->d_name, &st, 0);
    }
...
if (to_copy > size) to_copy = size; memcpy(buf, content + offset, to_copy); free(content);
...
int res = pread(fd, buf, size, offset);
	...
	if (res == -1) res = -errno; close(fd);

    return res;
}

static struct fuse_operations xmp_oper = { .getattr = xmp_getattr,.readdir = xmp_readdir,.open    = xmp_open,.read    = xmp_read,
...
if (realpath(argv[1], source_dir) == NULL) { perror("realpath");
...
char *fuse_argv[argc]; fuse_argv[0] = argv[0]; fuse_argv[1] = argv[2];
